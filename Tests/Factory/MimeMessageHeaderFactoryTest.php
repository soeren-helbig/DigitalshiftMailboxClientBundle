 ...
 */
class MimeMessageHeaderFactoryTest extends BaseTestCase
{
    ...
    {
        $messageHeaderFactory = $this->getMessageHeaderFactory();
        $header = $messageHeaderFactory->byArray($array);
    ...
    {
        $header1 = $this->getExpectedMessageHeader(1);
        $header2 = $this->getExpectedMessageHeader(2);
     ...
     * @param array $headers
     */
    public function testByMimePart(MimeMessagePart $mimePart, $headers)
    {
        $messageHeaderFactory = $this->getMessageHeaderFactory();
        $header = $messageHeaderFactory->byMimePart($mimePart);
    ...
    public function mimePartProvider()
    {
        ...
        return array(
            array(new MimeMessagePart($header1), $header1),
            array(new MimeMessagePart($header2), $header2),
        );
    }

    /**
     * @return MimeMessageHeaderFactory
     */
    private function getMessageHeaderFactory()
    {
        return new MimeMessageHeaderFactory();
    }
     ...
     * @return null|array
     */
    private function getExpectedMessageHeader($messageNumber)
    {
        if ($messageNumber === 1) {
            return array(
